...
class seasons extends connection{
    ...
    private $seriesid;
    private $stitle;
    public function series_trailer($SID,$STT){
        $this->seriesid = $SID;
        $this->stitle = $STT;
        $sqll = "SELECT trailer FROM d_tvseries WHERE id = '$this->seriesid' AND deleted_at IS NULL";
        ...
        if($go){
            ...
            <video width="100%" height="500px" controls autoplay controlsList="nodownload">
                <source src="niiflicks/directors/uploads/tvseries/trailers/'.$trailer['trailer'].'" type="video/mp4">
    ...
    public function series_detail(){
        $sqlm = "SELECT title,genre,view_count,description,release_date FROM d_tvseries WHERE id = '$this->seriesid' AND deleted_at IS NULL";
        ...
        if($gom){
            ...
            <h2 class="gen-title">'.$dserie['title'].'</h2>
